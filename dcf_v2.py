...
def get_supabase_data():
    ...
    calc_metrics_quarter_data = supabase.table("idx_calc_metrics_quarterly").select("symbol","total_liabilities_mrq","total_debt_mrq","total_equity_mrq","diluted_eps_ttm","avg_diluted_shares_ttm").execute()
...
for _, row in data.iterrows():
    ...
    if response.data[0]['intrinsic_value'] is not None:
        print(f"Upserted data for symbol: {row['symbol']}")
    else:
        print(f"Error upserting data for symbol: {row['symbol']}")
